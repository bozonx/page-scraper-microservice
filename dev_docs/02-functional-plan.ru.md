...
по заершению батча все данные удаляются из памяти через заданное время в переменной окружения `DATA_LIFETIME_MINS` (по умолчанию `60`). по истечению этого времени никаких данных об этом батче не должно остаться в памяти. используется просто таймаут, без нешнего планировщика.