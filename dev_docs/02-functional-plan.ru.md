...
**Важно:** Сервис не хранит никаких данных, только врменно хранит таски. Он только обрабатывает запросы и возвращает ответы. Для управления задачами используется встроенная очередь в Crawlee, которая позволяет выполнять несколько заданий одновременно.
...
- `DEFAULT_BATCH_CONCURRENCY` — количество одновременных запросов (по умолчанию `1`)
...
по заершению батча все данные удаляются из памяти через заданное время в переменной окружения `BATCH_DATA_LIFETIME_MINS` (по умолчанию `60`). по истечению этого времени никаких данных об этом батче не должно остаться в памяти. используется просто таймаут, без нешнего планировщика.